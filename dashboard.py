...
.hero-title {
    ...
    animation: shimmer 4s linear infinite, fade-in 1s ease;
}
...
.sec-head-line {
    flex: 1;
    height: 1px;
    background: linear-gradient(90deg, #1a1a35, transparent);
}
...
.neon-divider {
    height: 1px;
    background: linear-gradient(90deg, transparent, #1a1a35, #8b5cf6, #1a1a35, transparent);
    margin: 1.5rem 0;
    position: relative;
}
...
(function() {
    function initParticles() {
        ...
        if (!canvas) { setTimeout(initParticles, 500); return; }
...
def load_data():
    result = supabase.table("youtube_videos").select("*").execute()
...
def check_clickbait(title, thumbnail_url):
    try:
        result = groq_client.chat.completions.create(
            ...
            messages=[{
                "role": "user",
                "content": f"""You are a YouTube clickbait detector.
Video Title: "{title}"
...
<div class="hero-wrap">
    ...
    <div class="hero-eyebrow">⚡ realtime analytics engine v2.0</div>
    <div class="hero-title">PULSEBOARD</div>
    <div class="hero-sub">// youtube intelligence dashboard • live supabase sync</div>
</div>
""", unsafe_allow_html=True)
...
<div class="metric-row">
    <div class="m-card">
        ...
        <div class="m-label">Total Videos</div>
        <div class="m-value">{len(df)}</div>
    </div>
    <div class="m-card">
        <span class="m-icon">👁️</span>
        <div class="m-label">Total Views</div>
        <div class="m-value">{df['views'].sum():,}</div>
    </div>
    <div class="m-card">
        <span class="m-icon">👍</span>
        <div class="m-label">Total Likes</div>
        <div class="m-value">{df['likes'].sum():,}</div>
    </div>
    <div class="m-card">
        <span class="m-icon">📊</span>
        <div class="m-label">Avg Views/Video</div>
        <div class="m-value">{df['views'].mean():.1f}</div>
    </div>
</div>
""", unsafe_allow_html=True)
...
<div class="sec-head">
    ...
    <div class="sec-head-text">📈 Views Over Time</div>
...
fig.add_trace(go.Bar(
    ...
    y=df_sorted["views"],
    marker=dict(
        color=df_sorted["views"],
        colorscale=[[0, "#1a1a35"], [0.4, "#4c1d95"], [0.7, "#8b5cf6"], [1, "#06b6d4"]],
        line=dict(width=0),
    ),
...
fig.update_layout(
    ...
    font=dict(color="#334155", family="JetBrains Mono"),
    showlegend=False,
...
<div class="sec-head">
    <div class="sec-head-dot"></div>
    <div class="sec-head-text">🏆 Top 5 Videos by Views</div>
    <div class="sec-head-line"></div>
</div>
""", unsafe_allow_html=True)
...
<div class="sec-head">
    <div class="sec-head-dot"></div>
    <div class="sec-head-text">🔍 Semantic Search</div>
    <div class="sec-head-line"></div>
</div>
""", unsafe_allow_html=True)
        ...
        for _, row in results.iterrows():
            ...
            with col2:
                ...
                <div class="v-card">
                    ...
                    <br>
                    <span class="match-pill">◈ Match Score: {match_pct}%</span>
                </div>
                """, unsafe_allow_html=True)
...
<div class="sec-head">
    <div class="sec-head-dot"></div>
    <div class="sec-head-text">📋 All Videos</div>
    <div class="sec-head-line"></div>
</div>
""", unsafe_allow_html=True)
